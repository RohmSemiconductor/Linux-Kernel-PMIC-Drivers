...
static ssize_t clk_en_store(struct kobject *ko, struct kobj_attribute *a, const char *b, size_t c)
{
	...
	if(c>1 && (*b == '1' || *b == '0'))
	{
		if(g_c && !IS_ERR(g_c))
		{
			...
			pr_err("clk_get(NULL, bd71815-32k-out) has FAILED (%d)\n",(rval=PTR_ERR(g_c)));
	};
	if(!rval && (rval=c))
		pr_info("YaY!, Clk '%s' %sbled\n","bd71815-32k-out",(*b == '0')?"disa":"ena");
	else
...
static int mva_test_probe(struct platform_device *pdev)
{
	...
	pr_info("%s: Hello Peeps\n",__func__);
		if((rv = create_sysfs_for_tests()))
			return rv;
	g_c = clk_get(&pdev->dev, "bd71815-32k-out");
	if(!g_c || IS_ERR(g_c))
	{
		pr_info("clk_get(&pdev->dev, \"bd71815-32k-out\"); did not work\n");
		if ( !(g_c = clk_get(NULL, "bd71815-32k-out") ) || IS_ERR(g_c))
		{
			pr_info("clk_get(NULL, \"bd71815-32k-out\"); did not work\n");
			...
			pr_info("YAY! clk_get(NULL, \"bd71815-32k-out\"); did work\n");
	}
	else
		pr_info("YAY clk_get(&pdev->dev, \"bd71815-32k-out\"); did work\n");
	return rv;
};
...
static const struct of_device_id bd71815_test_of_match[] = {
        {
                .compatible = "rohm,clktest-bd71815",
        },
        { }
};
MODULE_DEVICE_TABLE(of, bd71815_test_of_match);

static struct platform_driver bd71815_test = {
        .driver = {
                .name = "bd718xx-test",
		.of_match_table = bd71815_test_of_match,
        },
        .probe = mva_test_probe,
	.remove = &mva_test_remove,
};

module_platform_driver(bd71815_test);

...
static int mva_test_probe(struct platform_device *pdev)
{
	...
	pr_info("%s: Hello Peeps\n",__func__);
		if((rv = create_sysfs_for_tests()))
			return rv;
	g_c = clk_get(&pdev->dev, "bd71847-32k-out");
	if(!g_c)
	{
		pr_info("clk_get(NULL, \"bd71847-32k-out\"); did not work\n");
		if ( !(g_c = clk_get(NULL, "bd71847-32k-out") ))
		{
			pr_info("clk_get(NULL, \"bd71847-32k-out\"); did not work\n");
			if ( !(g_c = clk_get(&pdev->dev, NULL) ))
				pr_info("clk_get(&pdev->dev, NULL); did not work\n");
			else
				pr_info("YAY clk_get(&pdev->dev, NULL); did work\n");
		}
		else
			pr_info("YAY! clk_get(NULL, \"bd71847-32k-out\"); did work\n");
	}
	else
...
MODULE_DEVICE_TABLE(of, bd71847_test_of_match);

static struct platform_driver bd71847_test = {
        .driver = {
                .name = "bd718xx-test",
		.of_match_table = bd71847_test_of_match,
        },
        .probe = mva_test_probe,
	.remove = &mva_test_remove,
};

module_platform_driver(bd71847_test);
